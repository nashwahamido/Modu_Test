...
export function AssemblyScreen() {
  ...
  const sceneRef = useRef<{ commitSnap: (step: AssemblyStep) => void } | null>(null);
  ...
  const gestures = useAssemblyGestures({
    onPickupProgress: (p: number, x: number, y: number) => setRingProgress(p),
    onPickupComplete: (x: number, y: number) => {
